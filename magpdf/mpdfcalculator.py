...
def calculateDr(r,fr,q,ff,paraScale=1.0,
...
#    fr*=orderedScale ## scaling already taken care of in calculatemPDF
...
class mPDFcalculator:
    ...
    def calc(self,normalized=True,both=False):
        ...
        rcalc,frcalc=calculatemPDF(self.magstruc.atoms,self.magstruc.spins,
                    ...
                    self.dampRate,self.dampPower,self.maxextension,self.ordScale)
    ...
    def plot(self,normalized=True,both=False):
        ...
        rcalc,frcalc=calculatemPDF(self.magstruc.atoms,self.magstruc.spins,
                     ...
                     self.dampRate,self.dampPower,self.maxextension,self.ordScale)
        if normalized and not both: 
            ax.plot(rcalc,frcalc*np.sqrt(2*np.pi)) # extra factor makes it easier to compare with d(r)
