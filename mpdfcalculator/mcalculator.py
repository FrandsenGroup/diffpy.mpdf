...
class magSpecies:
    ...
    """
    def __init__(self,struc=[],label='',magIdxs=[0],atoms=np.array([]),spins=np.array([]),rmaxAtoms=30.0,basisvecs=np.array([[0,0,1]]),kvecs=np.array([[0,0,0]]),gS=2.0,gL=0.0,ffparamkey=None,ffqgrid=np.array([0]),ff=np.array([])):
        self.struc=struc
        self.label=label
    ...
    def makeFF(self):    
        g=self.gS+self.gL
        if 0*getFFparams(self.ffparamkey) == []:
...
class magStructure:
    ...
    def __init__(self,struc=[],species={},atoms=np.array([]),spins=np.array([]),rmaxAtoms=30.0):
        self.struc=struc        
